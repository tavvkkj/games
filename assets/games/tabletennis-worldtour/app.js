...
var hasLockPointer = window.famobi.hasFeature("lockPointer");
        ...
        UserInput.prototype.move = function (e, _posX, _posY, _identifer, _isDown) {
            ...
            if(!isMobile && userBat != undefined && !firstRun) {
                if(document.pointerLockElement === canvas || document.mozPointerLockElement === canvas)
                {
                  ...
                  {
                    if((window.famobi.pointerLockHelper.mousePos.x+movementX) < window.innerWidth && (window.famobi.pointerLockHelper.mousePos.x+movementX) > 0)
                      window.famobi.pointerLockHelper.mousePos.x += movementX;
                    if((window.famobi.pointerLockHelper.mousePos.y+movementY) < window.innerHeight && (window.famobi.pointerLockHelper.mousePos.y+movementY) > 0)
...
function initSplash() {
    ...
    window.famobi.onRequest("resumeGameplay", function() {
        if(gameState == "pause") {
            butEventHandler("playFromPause", true);
        }
    });
...
function initStartScreen() {
    ...
    if(window.famobi.hasFeature("credits")) {
        ...
        aButs = new Array(oMoreGamesBut, oCreditsBut, oPlayBut);
    }
...
function _initGame() {
    ...
    if(hasLockPointer)
    {
      ...
      window.famobi.pointerLockHelper.mousePos = {x: window.innerWidth/2, y:window.innerHeight/2};
...
function updateScore(_player) {
    ...
    if(_player == "user") {
        ...
        playSound("userPoint");
        if((oGameData.userScore >= 11 && oGameData.enemyScore <= oGameData.userScore - 2) || oGameData.userScore == 99) {
            initGameComplete();
        }
    } else {
        ...
        if((oGameData.enemyScore >= 11 && oGameData.userScore <= oGameData.enemyScore - 2) || oGameData.enemyScore == 99) {
            initGameComplete();
        }
    }
}
